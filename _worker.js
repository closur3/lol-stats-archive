// ====================================================
// 🥇 Worker V39.6.0: 交互式数据深度探索与内核修正
// 更新特性:
// 1. 深度交互：新增 BO3/BO5/Series 列的点击穿透功能，支持按赛制过滤历史战绩，提供更细粒度的数据洞察
// 2. 内核修正：修复客户端脚本在 Worker 环境下的模板字符串转义漏洞，彻底解决交互函数 (openStats) 未定义的问题
// 3. 视觉微调：重构页脚状态栏布局 (Flexbox) 实现图标绝对垂直居中；优化战绩弹窗比分间距，提升阅读体验
// 4. 逻辑校准：修正调度系统日志语义，将 Newday 触发器准确描述为 "Daily check triggered"，消除误导性状态提示
// ====================================================
...
async function runUpdate(env, force=false) {
    ...
    runtimeConfig.TOURNAMENTS.forEach(t => {
        ...
        if (force || elapsed >= threshold || isNewDay) {
            if (isNewDay) {
                l.info(`🌅 Newday: ${t.slug} Force daily check triggered`);
            }
            candidates.push({ 
                slug: t.slug, 

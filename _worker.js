// ====================================================
// 🥇 Worker V36.2.59: 按钮增强 & 色彩同步版
// 基于: V36.2.58
// 变更: 
// 1. 增强队伍名称的按钮质感 (Padding加大, Hover更加明显)
// 2. TAB栏 (分组表头) 背景色改为动态读取，与卡片表头 (Header) 保持完全一致 (蓝/灰)
// ====================================================

const UI_VERSION = "2026-02-04-V36.2.59-ButtonSync-2";
...
const PYTHON_STYLE = `
    ...
    /* 核心变更: 增强按钮质感 (Padding + Hover) */
    .team-text { font-weight: 800; color: #334155; padding: 2px 8px; border-radius: 4px; transition: all 0.2s; display: inline-block; }
    .team-clickable { cursor: pointer; } 
    .team-clickable:hover { color: #2563eb; background-color: #eff6ff !important; transform: translateY(-1px); }
    ...
    .sch-container { 
        ...
        /* 核心变更: 移除 align-items: start，恢复等高拉伸 */
    }
    ...
    /* TAB: 极简线条风 (Minimalist Divider) - 颜色会在HTML中动态覆盖 */
    .sch-group-header { 
...
function renderFullHtml(globalStats, timeData, updateTime, debugInfo, maxDateTs, statusText, scheduleMap, runtimeConfig) {
    ...
    if (dates.length === 0) {
        ...
        dates.forEach(d => {
            ...
            // 核心变更: 在 sch-group-header 的 style 中动态应用 titleBg，实现颜色同步
            ...
            matches.forEach(m => {
                ...
                const t1Class = isTbd1 ? "team-text" : "team-text team-clickable";
                const t2Class = isTbd2 ? "team-text" : "team-text team-clickable";

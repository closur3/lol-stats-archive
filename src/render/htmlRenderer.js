...
export class HTMLRenderer {
  ...
  static renderFontLinks() {
    ...
    * 渲染导航栏 HTML
    */
  static renderNavBar(activeMode = "home") {
    const navItems = [
      { href: "/", label: "Home" },
    ...
    return `
<header class="main-header"><div class="nav-container"><div class="nav-left"><span class="nav-logo">🥇</span><h1 class="nav-title">LoL Stats</h1></div><div class="nav-right"><nav class="nav-links">${desktopNav}</nav><button class="nav-toggle" onclick="document.getElementById('mobileMenu').classList.add('open');document.getElementById('mobileOverlay').classList.add('open');document.body.classList.add('nav-mobile-open')" aria-label="Open menu"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"/></svg></button></div></div></header>
...
<div class="nav-mobile-menu" id="mobileMenu"><div class="nav-mobile-header"><button class="nav-mobile-close" onclick="document.getElementById('mobileMenu').classList.remove('open');document.getElementById('mobileOverlay').classList.remove('open');document.body.classList.remove('nav-mobile-open')" aria-label="Close menu"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button></div><nav class="nav-mobile-links">${mobileNav}</nav></div>`;
  ...
  static renderPageShell(title, bodyContent, navMode = "home", time = null, sha = null) {
    return `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>${title}</title>${HTMLRenderer.renderFontLinks()}<style>${PYTHON_STYLE}${BUILD_FOOTER_STYLE}</style><link rel="icon" href="/favicon.ico"></head><body>${HTMLRenderer.renderNavBar(navMode)}<div class="container">${bodyContent}</div>${HTMLRenderer.renderBuildFooter(time, sha)}<div id="matchModal" class="modal"><div class="modal-content"><h3 id="modalTitle">Match History</h3><div id="modalList" class="match-list"></div></div></div>${HTMLRenderer.renderPythonJS()}</body></html>`;
  }
  ...
  static renderToolsPage(time, sha, existingArchives = []) {
    ...
    <body>
        ...
        ${HTMLRenderer.renderNavBar("tools")}
...
<body>
    ${HTMLRenderer.renderNavBar("logs")}
